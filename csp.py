...
class CSP(search.Problem):
    ...
    def __init__(self, variables, domains, neighbors, constraints, const):
        ...
        #for cbj
        self.last_variable=None # save the last variable
        self.prev_conflict=[]   # list holding the set of coflict of the last variable
        self.conflict_set={}    # dictionary  holding the conflict set of every variable
...
def domwdeg(assignment, csp):
    # returns the variable with the minimum fraction, just like the article says you do total_weight[var] += csp.weight[(var,neigh)]
    # when both variables of the constraint are not in assignment
    csp.support_pruning()
        ...
        if var not in assignment:
            ...
            for neigh in csp.neighbors[var]:
                if neigh not in assignment:
                    total_weight[var] += csp.weight[(var,neigh)]
            remaining_domain = len(csp.curr_domains[var])
            if remaining_domain > 0:
                fraction_calc = remaining_domain / total_weight[var]
...
def backtracking_search(csp, select_unassigned_variable,
    ...
    return result,csp.Total_Checks


def cbj_algorithm(csp,select_unassigned_variable = domwdeg,
    ...
    def cbj(assignment):
        ...
        for value in order_domain_values(var, assignment, csp):
            if 0 == csp.nconflicts(var, value, assignment):
                # Assign the variable and update conflict information 
                csp.assign(var, value, assignment)
                ...
                # Perform fc and update total_checks
                case_result, check = inference(csp, var, value, assignment, removals)
                csp.Total_Checks += check
                ...
                # Restore the assignment for backtracking
                csp.restore(removals)

            # Check if the variable is in the conflict set of the last variable, if yes then merge the sets
            if csp.last_variable is not None and var in csp.conflict_set[csp.last_variable]:
                csp.last_variable = None
                for neighbor in csp.neighbors[var]:
                    if neighbor == var:
                        csp.conflict_set[neighbor] += csp.prev_conflict
                    csp.conflict_set[neighbor] = [variable for variable in assignment if variable in csp.conflict_set[neighbor]]

            csp.unassign(var, assignment)

    
            # Check if a dead-end was encountered in the last variable
            if csp.last_variable is not None:
                return None

        # Save the current variable as the last variable with its conflict set
